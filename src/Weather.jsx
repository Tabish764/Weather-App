...
const Weather = () => {
  ...
  const getWeather = async (city) => {
    ...
    const response = await axios.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${"baf006be54d61b49b009cdf85a7f80e0"}`
    );
      ...
      <div className="weather bg-gradient-to-r from-lime-600 max-w-[90%] sm:w-[400px] w-[280px] to-green-700 flex justify-center items-center sm:p-[40px] p-[10px] rounded-xl flex-col ">
        <div className="search-bar flex sm:flex items-center gap-3">
          <input
            ...
            className="sm:h-[50px] sm:w-[280px] w-[180px] border-none outline-none rounded-full pl-[20px] mt-5 sm:pl-[25px] text-black text-2xl"
            type="text"
            value={city}
            placeholder="Search"
          />
          <img
            className="sm:w-[50px] w-[30px] p-[7px] sm:p-[15px] mt-5 rounded-full cursor-pointer bg-white"
            src={search_icon}
            alt=""
            onClick={handleClick}
          />
        </div>
        <div>{error && <p className="font-medium text-white mt-5">{error}</p>}</div>
        <div className="text-center flex-wrap flex flex-col items-center justify-center">
          <img
            src={weather.icon || clear_icon}
            alt="Weather Icon"
            className="w-[150px] mt-[30px]"
          />
          <p className="text-white sm:text-7xl text-5xl ">
            {weather.temperature}°C
          </p>
          <p className="text-white sm:text-4xl text-2xl mt-2">
            {weather.location}
          </p>
          <div className="">
            <div className="flex   mt-[60px] gap-[96px]   text-white">
              ...
              <div className="col sm:w-[150px] w-[110px] flex gap-[15px] items-center">
                ...
                <div>
                  <p className="sm:text-2xl text-[15px]">
